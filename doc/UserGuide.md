...
> The search is case sensitive, the order of the keywords does not matter, only tags are searched, 
and persons matching at least one keyword will be returned (i.e. `OR` search).

Examples: 
* `findtag friends`<br>
  Returns `John Doe` but not `Friends`, if John Doe has a tag `friends`
* `find friends oweMoney school`<br>
  Returns Any person having tags `friends`, `oweMoney`, or `school`
